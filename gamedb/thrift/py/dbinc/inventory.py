...
class InventoryMixin:
    ...
    def update_inventory(
        ...
        logger.info(
            f"=== UPDATE_INVENTORY: database={database}, id={obj.id}, "
            f"max_entries={obj.max_entries}, max_volume={obj.max_volume}"
        )
        logger.debug(f"Inventory has {len(obj.entries)} entries to update")
        try:
            if obj.id is None:
                logger.warning("Cannot update Inventory with id=None")
                return [
                    GameResult(
                        status=StatusType.FAILURE,
            ...
            # Start transaction
            logger.debug("Starting transaction for inventory update")
            cursor.execute("START TRANSACTION;")
            ...
            logger.debug(f"Owner type: {owner_type}, owner_id: {owner_id}")

            # Update inventory
            logger.debug(f"Updating inventory record id={obj.id}")
            cursor.execute(
            ...
            logger.debug(f"Deleting existing entries for inventory_id={obj.id}")
            cursor.execute(
            ...
            logger.debug(f"Updating owner record for inventory_id={obj.id}")
            cursor.execute(
            ...
            logger.info(
                f"SUCCESS: Updated inventory id={obj.id} with {len(obj.entries)} entries"
            )
            return [
                GameResult(
                    status=StatusType.SUCCESS,
            ...
            logger.error(f"EXCEPTION in update_inventory: {type(e).__name__}: {str(e)}")
            if self.connection:
                logger.debug("Rolling back transaction")
                self.connection.rollback()
            return [
    ...
    def load_inventory(
        ...
        logger.info(
            f"=== LOAD_INVENTORY: database={database}, inventory_id={inventory_id}"
        )
        try:
            self.connect()
            ...
            # Load the inventory
            logger.debug(f"Querying inventory table for id={inventory_id}")
            cursor.execute(
            ...
            if not inv_row:
                logger.warning(f"Inventory id={inventory_id} not found in database")
                cursor.close()
                return (
                    GameResult(
                        status=StatusType.FAILURE,
            ...
            logger.debug(f"Loading entries for inventory_id={inventory_id}")
            cursor.execute(
                f"SELECT * FROM {database}.{inventory_entries_table} WHERE inventory_id = %s;",
                (inventory_id,),
            )
            entry_rows = cursor.fetchall()
            logger.debug(f"Found {len(entry_rows)} entries")
            ...
            logger.info(
                f"SUCCESS: Loaded inventory id={inventory_id} with {len(entries)} entries"
            )
            return (
                GameResult(
                    status=StatusType.SUCCESS,
            ...
            logger.error(f"EXCEPTION in load_inventory: {type(e).__name__}: {str(e)}")
            return (
                GameResult(
                    status=StatusType.FAILURE,
    ...
    def save_inventory(
        ...
        logger.info(f"=== SAVE_INVENTORY: database={database}, id={obj.id}")
        if obj.id is None:
            logger.debug("Decision: CREATE inventory (id is None)")
            return self.create_inventory(database, obj, table)
        else:
            logger.debug(f"Decision: UPDATE inventory (id={obj.id})")
    ...
    def destroy_inventory(
        ...
        logger.info(
            f"=== DESTROY_INVENTORY: database={database}, inventory_id={inventory_id}"
        )
        try:
            self.connect()
            cursor = self.connection.cursor()
            ...
            # Start transaction
            logger.debug("Starting transaction for inventory destruction")
            cursor.execute("START TRANSACTION;")
            ...
            if cursor.rowcount == 0:
                logger.warning(
                    f"Inventory id={inventory_id} not found, nothing to delete"
                )
                self.connection.rollback()
                return [
                    GameResult(
            ...
            logger.info(f"SUCCESS: Destroyed inventory id={inventory_id}")
            return [
                GameResult(
                    status=StatusType.SUCCESS,
            ...
            logger.error(
                f"EXCEPTION in destroy_inventory: {type(e).__name__}: {str(e)}"
            )
            if self.connection:
                logger.debug("Rolling back transaction")
                self.connection.rollback()
    ...
    def list_inventory(
        ...
        try:
            ...
            logger.debug(f"query={query}, rpp={results_per_page}, offset={offset}")
            ...
            for row in inventory_rows:
                # Load inventory entries
                inventory_entries_table = "inventory_entries"
                statement = f"SELECT * FROM {database}.{inventory_entries_table} WHERE inventory_id = %s;"
                logger.debug(f"{statement}, {row['id']}")
                cursor.execute(
                    statement,
                    (row["id"],),
                )
