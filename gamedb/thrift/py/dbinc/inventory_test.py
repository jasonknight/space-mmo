...
def test_list_inventory(db: DB, database_name: str):
    ...
    for i in range(100):
        ...
        save_results = db.save_inventory(database_name, inventory)
        assert is_ok(save_results), f"Failed to save inventory {i}: {save_results[0].message}"
    ...
    while True:
        result, inventories, total_count = db.list_inventory(
            database_name,
            page,
            results_per_page,
        )
        assert is_true(result), f"Failed to list inventories page {page}: {result.message}"

        if not inventories:
            break

        all_paginated_inventories.extend(inventories)
        print(f"    Page {page}: retrieved {len(inventories)} inventories (total: {total_count})")
    ...
    assert len(all_paginated_inventories) == 100, f"Expected 100 inventories, got {len(all_paginated_inventories)}"
    print(f"  ✓ Pagination test passed: retrieved all {len(all_paginated_inventories)} inventories across {page + 1} pages")
...
def main():
    ...
    try:
        ...
        test_list_inventory(db, database_name)
